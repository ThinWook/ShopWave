...
### API configuration for Products page

The Products page now fetches data from your backend using `src/utils/apiClient.ts`.

- Set the API base URL via an environment variable in a `.env` file at the project root:

  ```env
  VITE_API_BASE_URL=https://your-api.example.com
  ```

- Expected endpoints (adjust in `src/services/productService.ts` if different):
  - `GET /api/v1/products?page=1&pageSize=7&q=keyword` returns either an array or a paginated object like `{ items: [...], total: 123 }`.
