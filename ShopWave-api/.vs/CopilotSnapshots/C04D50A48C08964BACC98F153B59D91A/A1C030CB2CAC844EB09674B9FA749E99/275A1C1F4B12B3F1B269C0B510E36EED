    ...
    public class ProductsController : ControllerBase
    {
                ...
                if (request.Variants != null && request.Variants.Count > 0)
                {
                    for (int i = 0; i < request.Variants.Count; i++)
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(sku))
                        {
                            ...
                            var categoryId = request.CategoryId.ToString("N").ToUpper();
                            var sizeSlug = ToSlug(v.Size ?? "");
                            var colorSlug = ToSlug(v.Color ?? "");
                            sku = $"{nameSlug}-{categoryId}-{sizeSlug}-{colorSlug}-{i + 1}";
                        }
        ...
        public async Task<IActionResult> GetProducts([FromQuery] ProductSearchRequest request)
        {
            try
            {
                ...
                var etagRaw = $"{totalRecords}-{latestUpdated:O}-{request.Page}-{request.PageSize}-{sortKey}-{request.SortDirection}-{request.SearchTerm}-{request.CategoryId}-{request.MinRating}";
                ...
                // Load variants for all products in page
                ...
                {
                    List<object> galleryImagesList = new List<object>();
                    if (p.ProductMedia != null)
                    {
                        galleryImagesList = p.ProductMedia
                            .OrderBy(pm => pm.DisplayOrder)
                            .Select(pm => (object)new { id = pm.Media.Id, url = pm.Media.Url, sortOrder = pm.DisplayOrder })
                            .ToList();
                    }

                    List<object> variantList = new List<object>();
                    if (variantsByProduct.TryGetValue(p.Id, out var list))
                    {
                        variantList = list.Select(v => (object)new
                        {
                            id = v.Id,
                            sku = v.Sku,
                            price = v.Price,
                ...
                var items = itemsList; // keep existing variable name for response
        ...
        public async Task<IActionResult> GetProduct(Guid id)
        {
            try
            {
                ...
                var variants = _context.ProductVariants.Where(v => v.ProductId == product.Id).Select(v => new
                {
                    id = v.Id,
                    sku = v.Sku,
                    price = v.Price,
        ...
        public async Task<IActionResult> UpdateProduct(Guid id, [FromBody] UpdateProductRequest request)
        {
            try
            {
                ...
                {
                    // Xóa các variants cũ không còn trong request
                    ...
                    // Thêm mới hoặc cập nhật variants
        ...
        public async Task<IActionResult> UploadVariantImage(Guid variantId, [FromForm] IFormFile file, [FromServices] IMediaService mediaService)
        {
            try
            {
                ...
                // Use UploadFileAsync to match IMediaService implementation
