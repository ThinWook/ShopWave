    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> Register([FromBody] RegisterRequest request)
        {
            try
            {
                ...
                {
                    return Conflict(EnvelopeBuilder.Fail<object>(HttpContext, "EMAIL_EXISTS", new[] { new ErrorItem("email", "Email ?ã ???c s? d?ng", "EMAIL_EXISTS") }, 409));
                }
        ...
        public async Task<IActionResult> Login([FromBody] LoginRequest request)
        {
            try
            {
                ...
                if (user == null || !BCrypt.Net.BCrypt.Verify(request.Password, user.PasswordHash))
                {
                    ...
                    return Unauthorized(EnvelopeBuilder.Fail<object>(HttpContext, "INVALID_CREDENTIALS", new[] { new ErrorItem("auth", "Email ho?c m?t kh?u không ?úng", "INVALID_CREDENTIALS") }, 401));
                }
        ...
        public async Task<IActionResult> RefreshToken([FromBody] RefreshTokenRequest request)
        {
            try
            {
                ...
                {
                    return BadRequest(EnvelopeBuilder.Fail<object>(HttpContext, "REFRESH_TOKEN_REQUIRED", new[] { new ErrorItem("refreshToken", "Refresh token là b?t bu?c", "REQUIRED") }, 400));
                }
                ...
                {
                    return Unauthorized(EnvelopeBuilder.Fail<object>(HttpContext, "REFRESH_TOKEN_INVALID", new[] { new ErrorItem("refreshToken", "Refresh token không h?p l? ho?c ?ã h?t h?n", "INVALID") }, 401));
                }
        ...
        public async Task<IActionResult> ResetPassword([FromBody] ResetPasswordRequest request)
        {
            try
            {
                ...
                {
                    return BadRequest(EnvelopeBuilder.Fail<object>(HttpContext, "TOKEN_INVALID", new[] { new ErrorItem("token", "Token không h?p l? ho?c ?ã h?t h?n", "TOKEN_INVALID") }, 400));
                }

                var user = await _context.Users.FindAsync(userId);
                if (user == null || !user.IsActive)
                {
                    return BadRequest(EnvelopeBuilder.Fail<object>(HttpContext, "USER_INVALID", new[] { new ErrorItem("user", "Ng??i dùng không h?p l?", "USER_INVALID") }, 400));
                }
        ...
        public async Task<IActionResult> LoginWithGoogle([FromBody] GoogleLoginRequest request)
        {
            try
            {
                ...
                {
                    return BadRequest(EnvelopeBuilder.Fail<object>(HttpContext, "ID_TOKEN_REQUIRED", new[] { new ErrorItem("idToken", "Thi?u idToken", "REQUIRED") }, 400));
                }

                var payload = await _google.ValidateIdTokenAsync(request.IdToken);
                if (payload == null)
                {
                    return Unauthorized(EnvelopeBuilder.Fail<object>(HttpContext, "GOOGLE_TOKEN_INVALID", new[] { new ErrorItem("idToken", "Google token không h?p l? ho?c email ch?a ???c xác minh", "INVALID") }, 401));
                }

                if (payload.EmailVerified != true)
                {
                    return Unauthorized(EnvelopeBuilder.Fail<object>(HttpContext, "EMAIL_NOT_VERIFIED", new[] { new ErrorItem("idToken", "Email Google ch?a ???c xác minh", "UNVERIFIED_EMAIL") }, 401));
                }
