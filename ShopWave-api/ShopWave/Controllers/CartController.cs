    ...
    public class CartController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetCart()
        {
            try
            {
                ...
                var cartEntities = await _context.CartItems.Include(ci => ci.Product).Include(ci => ci.ProductVariant)
        ...
        public async Task<IActionResult> AddToCart([FromBody] AddToCartRequest request)
        {
            try
            {
                ...
                {
                    selectedVariant = await _context.ProductVariants.FirstOrDefaultAsync(v => v.Id == request.VariantId.Value && v.ProductId == product.Id);
                    if (selectedVariant == null)
                    {
                        return BadRequest(EnvelopeBuilder.Fail<object>(HttpContext, "INVALID_VARIANT", new[] { new ErrorItem("variant", "Variant not found for product", "INVALID_VARIANT") }, 400));
                    }
                ...
                {
                    var productVariants = await _context.ProductVariants.Where(v => v.ProductId == product.Id).OrderBy(v => v.Price).ToListAsync();
                    if (productVariants.Count == 0)
                    {
                        return BadRequest(EnvelopeBuilder.Fail<object>(HttpContext, "NO_VARIANT", new[] { new ErrorItem("variant", "No variants available", "NO_VARIANT") }, 400));
                    }
                ...
                Guid? variantId = selectedVariant?.Id;
                var existing = await _context.CartItems.FirstOrDefaultAsync(ci => ci.UserId == userId && ci.ProductId == request.ProductId && ci.ProductVariantId == variantId);
                if (existing != null)
                {
                ...
                {
                    var cartItem = new CartItem { UserId = userId, ProductId = request.ProductId, ProductVariantId = variantId, Quantity = request.Quantity, UnitPrice = selectedVariant?.Price ?? 0, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow };
        ...
        public async Task<IActionResult> UpdateCartItem(Guid id, [FromBody] UpdateCartItemRequest request)
        {
            try
            {
                ...
                var cartItem = await _context.CartItems.Include(ci => ci.ProductVariant).FirstOrDefaultAsync(ci => ci.Id == id && ci.UserId == userId);
                if (cartItem == null)
                {
